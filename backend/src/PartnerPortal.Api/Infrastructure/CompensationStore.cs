...
public sealed class CompensationStore
{
    ...
    private static void EnsureSchema(SqliteConnection c)
    {
        ...
        AddColumnIfMissing(c, "Invoices", "DocumentType", "TEXT NOT NULL DEFAULT 'Factura'");
        AddColumnIfMissing(c, "Invoices", "Perception", "NUMERIC NOT NULL DEFAULT 0");
    ...
    public void SeedDemoData(Guid partnerId, bool includeDemoInvoices = true)
    {
        lock (sync)
        {
            ...
            if (!includeDemoInvoices)
            {
                ...
                using var addDemoDeposits = c.CreateCommand(); addDemoDeposits.Transaction = tx; addDemoDeposits.CommandText = "INSERT INTO Credits(Id,PartnerId,Reference,DocumentType,DocumentDate,OriginalAmount,AvailableAmount) SELECT $d1,$p,'SALDO-A-FAVOR','Depósito bancario','2026-08-20T00:00:00.0000000',25000,25000 WHERE NOT EXISTS (SELECT 1 FROM Credits WHERE PartnerId=$p AND Reference='SALDO-A-FAVOR' AND DocumentType='Depósito bancario' AND OriginalAmount=25000 AND AvailableAmount>0); INSERT INTO Credits(Id,PartnerId,Reference,DocumentType,DocumentDate,OriginalAmount,AvailableAmount) SELECT $d2,$p,'SALDO-A-FAVOR','Depósito bancario','2026-08-20T00:00:00.0000000',30000,30000 WHERE NOT EXISTS (SELECT 1 FROM Credits WHERE PartnerId=$p AND Reference='SALDO-A-FAVOR' AND DocumentType='Depósito bancario' AND OriginalAmount=30000 AND AvailableAmount>0);"; addDemoDeposits.Parameters.AddWithValue("$p", partnerId.ToString()); addDemoDeposits.Parameters.AddWithValue("$d1", Guid.NewGuid().ToString()); addDemoDeposits.Parameters.AddWithValue("$d2", Guid.NewGuid().ToString()); addDemoDeposits.ExecuteNonQuery();
            }
            ...
            if (!includeDemoInvoices)
            {
                InsertInvoiceIfMissing(c, tx, partnerId, "01-F326-00086556",
                    new DateTime(2026, 8, 21), new DateTime(2026, 8, 22),
                    24866.42m, 497.33m);
                InsertInvoiceIfMissing(c, tx, partnerId, "01-F326-00086557",
                    new DateTime(2026, 8, 21), new DateTime(2026, 8, 22),
                    11910.03m, 238.20m);
            }
            tx.Commit();
    ...
    private static void InsertInvoice(SqliteConnection c, SqliteTransaction tx, Guid partnerId, string reference, DateTime issue, DateTime due, decimal amount, decimal perception) { var total = amount + perception; using var cmd=c.CreateCommand(); cmd.Transaction=tx; cmd.CommandText="INSERT INTO Invoices(Id,PartnerId,Reference,DocumentType,TotalAmount,Perception,PendingAmount,IssueDate,DueDate,Status) VALUES($id,$p,$r,'Factura',$total,$perception,$total,$issue,$due,'PENDING')"; cmd.Parameters.AddWithValue("$id",Guid.NewGuid().ToString()); cmd.Parameters.AddWithValue("$p",partnerId.ToString()); cmd.Parameters.AddWithValue("$r",reference); cmd.Parameters.AddWithValue("$total",total); cmd.Parameters.AddWithValue("$perception",perception); cmd.Parameters.AddWithValue("$issue",issue.ToString("O")); cmd.Parameters.AddWithValue("$due",due.ToString("O")); cmd.ExecuteNonQuery(); }
    private static void InsertInvoiceIfMissing(SqliteConnection c, SqliteTransaction tx, Guid partnerId, string reference, DateTime issue, DateTime due, decimal amount, decimal perception) { using var check=c.CreateCommand(); check.Transaction=tx; check.CommandText="SELECT COUNT(*) FROM Invoices WHERE PartnerId=$p AND Reference=$r"; check.Parameters.AddWithValue("$p",partnerId.ToString()); check.Parameters.AddWithValue("$r",reference); if(Convert.ToInt32(check.ExecuteScalar())==0) InsertInvoice(c,tx,partnerId,reference,issue,due,amount,perception); }
    ...
    public CompensationResponse Get(Guid partnerId)
    {
        lock (sync)
        {
            ...
            using(var cmd=c.CreateCommand()) { cmd.CommandText="SELECT Id,Reference,DocumentType,IssueDate,DueDate,TotalAmount,Perception,PendingAmount,Status FROM Invoices WHERE PartnerId=$p AND PendingAmount>0 AND Status NOT IN ('PAID','CANCELLED','COMPENSATED') ORDER BY DueDate,Id"; cmd.Parameters.AddWithValue("$p",partnerId.ToString()); using var r=cmd.ExecuteReader(); while(r.Read()){var due=DateTime.Parse(r.GetString(4)); invoices.Add(new(r.GetString(0),r.GetString(1),r.GetString(2),DateTime.Parse(r.GetString(3)),due,r.GetDecimal(5),r.GetDecimal(6),r.GetDecimal(7),r.GetString(8),due.Date<today));} }
            var credits=new List<CompensationCreditResponse>(); using(var cmd=c.CreateCommand()){cmd.CommandText="SELECT Id,Reference,DocumentType,DocumentDate,OriginalAmount,AvailableAmount FROM Credits WHERE PartnerId=$p AND AvailableAmount>0 ORDER BY CASE WHEN Reference='Sin ref.' AND DocumentType='Saldo a favor' THEN 0 ELSE 1 END, DocumentDate,Id";cmd.Parameters.AddWithValue("$p",partnerId.ToString());using var r=cmd.ExecuteReader();while(r.Read())credits.Add(new(r.GetString(0),r.GetString(1),r.GetString(2),DateTime.Parse(r.GetString(3)),r.GetDecimal(4),r.GetDecimal(5)));}
    ...
    public CompensationExecutionResponse Execute(Guid partnerId, Guid appliedBy, IReadOnlyCollection<Guid> invoiceIds, IReadOnlyCollection<Guid>? creditIds, string? observation, string idempotencyKey)
    {
        ...
        lock(sync)
        {
            ...
            using(var cmd=c.CreateCommand()){cmd.Transaction=tx;cmd.CommandText=ids is null?"SELECT Id,AvailableAmount FROM Credits WHERE PartnerId=$p AND AvailableAmount>0 ORDER BY CASE WHEN Reference='Sin ref.' AND DocumentType='Saldo a favor' THEN 0 ELSE 1 END, DocumentDate,Id":$"SELECT Id,AvailableAmount FROM Credits WHERE PartnerId=$p AND AvailableAmount>0 AND Id IN ({string.Join(',',ids.Select((_,i)=>"$c"+i))}) ORDER BY CASE WHEN Reference='Sin ref.' AND DocumentType='Saldo a favor' THEN 0 ELSE 1 END, DocumentDate,Id";cmd.Parameters.AddWithValue("$p",partnerId.ToString());if(ids is not null)for(var i=0;i<ids.Length;i++)cmd.Parameters.AddWithValue("$c"+i,ids[i].ToString());using var r=cmd.ExecuteReader();while(r.Read())credits.Add((Guid.Parse(r.GetString(0)),r.GetDecimal(1)));}
            ...
            if (ids is not null && ids.Length == credits.Count)
            {
                using var sinRefQuery = c.CreateCommand(); sinRefQuery.Transaction = tx; sinRefQuery.CommandText = "SELECT Id FROM Credits WHERE PartnerId=$p AND Reference='Sin ref.' AND DocumentType='Saldo a favor' LIMIT 1"; sinRefQuery.Parameters.AddWithValue("$p", partnerId.ToString()); var sinRefValue = sinRefQuery.ExecuteScalar()?.ToString();
                if (Guid.TryParse(sinRefValue, out var sinRefId) && ids.Contains(sinRefId))
                {
                    foreach (var credit in credits.Where(x => x.Id != sinRefId))
                    {
                        using (var merge = c.CreateCommand()) { merge.Transaction = tx; merge.CommandText = "UPDATE Credits SET AvailableAmount=AvailableAmount+$a WHERE Id=$i"; merge.Parameters.AddWithValue("$a", credit.Available); merge.Parameters.AddWithValue("$i", sinRefId.ToString()); merge.ExecuteNonQuery(); }
                        using (var hide = c.CreateCommand()) { hide.Transaction = tx; hide.CommandText = "UPDATE Credits SET AvailableAmount=0 WHERE Id=$i"; hide.Parameters.AddWithValue("$i", credit.Id.ToString()); hide.ExecuteNonQuery(); }
                    }
                }
            }
            using (var refreshRemainingCreditDate = c.CreateCommand())
            {
                ...
                refreshRemainingCreditDate.CommandText = "UPDATE Credits SET DocumentDate=$date WHERE PartnerId=$p AND Reference='Sin ref.' AND DocumentType='Saldo a favor' AND AvailableAmount>0";
                refreshRemainingCreditDate.Parameters.AddWithValue("$date", now.ToString("O"));
                refreshRemainingCreditDate.Parameters.AddWithValue("$p", partnerId.ToString());
                refreshRemainingCreditDate.ExecuteNonQuery();
            }
