...
public sealed class CompensationStore
{
    ...
    public CompensationExecutionResponse Execute(Guid partnerId, Guid appliedBy, IReadOnlyCollection<Guid> invoiceIds, IReadOnlyCollection<Guid>? creditIds, string? observation, string idempotencyKey)
    {
        ...
        lock(sync)
        {
            ...
            var previous=ScalarDecimal(c,tx,"SELECT COALESCE(SUM(AvailableAmount),0) FROM Credits WHERE PartnerId=$p AND AvailableAmount>0",("$p",partnerId.ToString())); var ids=creditIds is null||creditIds.Count==0?null:creditIds.Distinct().ToArray(); var credits=new List<(Guid Id,decimal Available)>();
