...
app.MapGet("/api/compensation", (HttpContext http, SessionStore sessions, CompensationStore store) => { if(!TryPartner(http,sessions,out var partnerId)) return Results.Unauthorized(); return Results.Ok(store.Get(partnerId)); });
app.MapGet("/api/compensation/history", (HttpContext http, SessionStore sessions, CompensationStore store) => { if(!TryPartner(http,sessions,out var partnerId)) return Results.Unauthorized(); return Results.Ok(store.History(partnerId)); });
app.MapPost("/api/compensation/execute", (ExecuteCompensationRequest request, HttpContext http, SessionStore sessions, CompensationStore store) => { if(!TryPartner(http,sessions,out var partnerId)) return Results.Unauthorized(); if(request.InvoiceIds is null || request.InvoiceIds.Count == 0 || request.InvoiceIds.Any(id=>!Guid.TryParse(id,out _)) || request.CreditIds?.Any(id=>!Guid.TryParse(id,out _)) == true) return Results.BadRequest(new { message="La selección de compensación no es válida." }); try { var result=store.Execute(partnerId,partnerId,request.InvoiceIds.Select(Guid.Parse).ToArray(),request.CreditIds?.Select(Guid.Parse).ToArray(),request.Observation,request.IdempotencyKey ?? Guid.NewGuid().ToString("N")); return Results.Ok(result); } catch(InvalidOperationException ex) { return Results.Conflict(new { message=ex.Message }); } });
app.Run();
static bool TryPartner(HttpContext http, SessionStore sessions, out Guid partnerId) { partnerId=Guid.Empty; return http.Request.Cookies.TryGetValue("access_token",out var token) && sessions.TryGet(token,out var session) && session is not null && (partnerId=session.PartnerId)!=Guid.Empty; }
