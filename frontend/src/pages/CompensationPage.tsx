...
export function CompensationPage() {
  ...
  return <main className="compensation-page">
    ...
    <section className="compensation-content"><div className="compensation-breadcrumb">HOME <b>›</b> MI NEGOCIO <b>›</b> <strong>COMPENSACIÓN</strong></div><h1><img src={compensationImage} alt="" /> Haz tu compensación</h1><p className="compensation-intro">Realiza aquí tu compensación para mantener tu cuenta corriente Solgas ordenada.</p>
      <div className="balance-cards"><div><strong>SALDO A FAVOR</strong><span>{money(data.totalAvailableCredit)}</span></div><div><strong>DEUDA TOTAL</strong><span>{money(0)}</span></div><div><strong>DEUDA VENCIDA</strong><span>{money(data.overdueDebt)}</span></div></div>
      <div className="invoice-section"><h2>PASO 1: SELECCIONA LAS FACTURAS A COMPENSAR</h2><table><thead><tr><th><input type="checkbox" aria-label="Seleccionar todas las facturas" checked={allInvoicesSelected} onChange={() => setSelectedInvoices(allInvoicesSelected ? [] : data.invoices.map(item => item.id))} /></th><th>Referencia</th><th>Fecha emisión</th><th>Vencimiento</th><th>Importe total</th><th>Saldo pendiente</th><th>Estado</th><th>Vencida</th></tr></thead><tbody>{data.invoices.length === 0 ? <tr><td colSpan={8}>No hay facturas pendientes.</td></tr> : data.invoices.map(item => <tr key={item.id}><td><input type="checkbox" aria-label={`Seleccionar factura ${item.reference}`} checked={selectedInvoices.includes(item.id)} onChange={() => toggleInvoice(item.id)} /></td><td><strong>{item.reference}</strong></td><td>{new Date(item.issueDate).toLocaleDateString('es-PE')}</td><td>{new Date(item.dueDate).toLocaleDateString('es-PE')}</td><td>{money(item.totalAmount)}</td><td>{money(item.pendingAmount)}</td><td>{item.status}</td><td>{item.isOverdue ? 'Sí' : 'No'}</td></tr>)}</tbody></table></div>
      ...
      <div className="compensation-step"><h2>PASO 3: REVISA EL RESUMEN DE LA COMPENSACIÓN</h2><div className="summary-row"><span>Facturas:</span><span>{money(invoiceTotal)}</span></div><div className="summary-row"><span>Notas de crédito:</span><span>{money(selectedCreditTotal)}{selectedCredits.length > 0 ? ` (en ${selectedCredits.length} documentos)` : ''}</span></div><div className="summary-row"><span>Saldo pendiente:</span><span>{money(invoiceTotal - selectedCreditTotal)}</span></div>{error && <div className="compensation-warning">{error}</div>}{message && <div className="compensation-success">{message}</div>}{!error && !message && selectedInvoices.length === 0 && <div className="compensation-warning">No puedes ejecutar la compensación, comienza seleccionando una o más facturas</div>}{!error && !message && selectedInvoices.length > 0 && invoiceTotal > selectedCreditTotal && <div className="compensation-warning">El saldo a favor no es suficiente para cubrir las facturas seleccionadas.</div>}</div>
